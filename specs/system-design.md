...
WitMate 是一个本地优先的个人 Agent 协调器，通过统一入口管理多个专家 Agent/Skill，并通过本地 qmd 文件系统积累长期记忆。
...
| `notification_ops` | 通知 | 仅 WitMate 内部触发 |
...
```json
{
  ...
  "capability": "file_ops",
  "input_summary": "读取文件: ~/projects/witmate/README.md",
  "result": "success",
  "duration_ms": 150
}
```
...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐ │
│  │  witmate-web  │  │  witmate-cli  │  │    witmate-server     │ │
