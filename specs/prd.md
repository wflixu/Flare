...
- **不做专业操作**：Flare 负责协调，不直接执行代码编写、图像生成等专业任务，而是调用专家 Agent
...
**使用场景**：
- 每天早晨：向 Flare 汇报今日计划，Flare 记录并提醒关键任务
- 需求分析：Flare 调用"分析师 Agent"协助梳理用户反馈
- 文档撰写：Flare 记录之前的讨论上下文，自动补充背景
- 周报撰写：Flare 从记忆中提取本周关键进展
...
**使用场景**：
- 文献综述：Flare 记录阅读笔记，自动关联相关文献
- 实验设计：Flare 从记忆中提取之前的实验方案供参考
- 论文写作：Flare 协调"写作 Agent"和"研究 Agent"

### User Stories

#### Epic 1: 记忆系统

**Story 1.1**: 作为知识工作者，我希望 Flare 能记住我之前的项目背景，这样我就不用每次都重复说明。
**Acceptance Criteria**:
- [ ] Given 我在上一会话中描述了项目背景，当我在新会话中提问相关问题时，Then Flare 能引用之前的背景信息
- [ ] Given 我有多个项目背景记忆，当我提问时，Then Flare 能识别并引用最相关的项目
- [ ] Performance: 记忆检索响应时间 < 2s（对于 1000 个记忆文件）

**Story 1.2**: 作为研究人员，我希望 Flare 能从我的对话中学习新知识，这样他能越来越懂我的研究领域。
**Acceptance Criteria**:
- [ ] Given 我在对话中提供了新的研究信息，当对话结束时，Then Flare 自动提取并存储关键知识
- [ ] Given Flare 存储了我的研究知识，当我询问相关问题时，Then Flare 能应用这些知识进行推理
- [ ] Edge case: 当我提供的信息与已有知识冲突时，Then Flare 询问确认而非直接覆盖

#### Epic 2: Agent & Skill/MCP 协调

**Story 2.1**: 作为用户，我希望 Flare 能识别我的需求并调用合适的专家 Agent/Skill，这样我能得到专业的帮助。
**Acceptance Criteria**:
- [ ] Given 我提出代码相关问题，当 Flare 分析后，Then 调用代码专家 Skill
- [ ] Given 我提出写作相关问题，当 Flare 分析后，Then 调用写作专家 Skill
- [ ] Given 我提出 PDF 处理需求，当 Flare 分析后，Then 调用 PDF Skill
- [ ] Given 我提出的需求涉及多个领域，当 Flare 分析后，Then 协调多个 Skill/MCP 协作
- [ ] Given Skill/MCP 调用失败，当错误发生时，Then Flare 提供清晰的错误信息并建议替代方案

**Story 2.2**: 作为用户，我希望看到 Flare 调用了哪些 Skill/MCP，这样我能理解它的决策过程。
**Acceptance Criteria**:
- [ ] Given Flare 调用了一个 Skill/MCP，当调用发生时，Then 在界面上显示名称和调用原因
- [ ] Given Flare 协调多个 Skill/MCP，当协调过程发生时，Then 显示协调流程图或时间线
- [ ] Given 用户希望详细了解，当用户点击详情时，Then 展示完整的推理链

**Story 2.3**: 作为高级用户，我希望能配置和添加新的 Skill/MCP，这样我能扩展 Flare 的能力。
**Acceptance Criteria**:
- [ ] Given 我有新的 MCP server 配置，当我在 config 中添加时，Then Flare 能连接并使用该 MCP
- [ ] Given 我创建自定义 skill 文件，当放置到 skill 目录时，Then Flare 能发现并加载
- [ ] Given skill/MCP 列表，当我查看时，Then 显示所有可用能力及其状态

#### Epic 3: 自我进化

**Story 3.1**: 作为用户，我希望 Flare 能记住我们的对话并越来越懂我，这样体验越来越好。
**Acceptance Criteria**:
- [ ] Given 我在对话中提到偏好（如"我喜欢简洁的回答"），当对话结束时，Then Flare 存储此偏好并在后续应用
- [ ] Given 我多次使用某 Skill，当达到阈值时，Then Flare 自动提升该 Skill 的推荐优先级
- [ ] Given 我纠正 Flare 的回答，当纠正发生时，Then Flare 学习并更新相关知识
- [ ] Given Flare 发现我的行为模式，当模式稳定时，Then 提炼为习惯规则

**Story 3.2**: 作为用户，我希望 Flare 有"做梦"机制定期整合和优化记忆，这样不会积累无用信息，系统持续进化。
**Acceptance Criteria**:
- [ ] Given 系统闲置超过 30 分钟，When 定时触发时，Then 自动启动 Dream 整合
- [ ] Given 每日凌晨（可配置），When 定时触发时，Then 自动启动 Dream 整合
- [ ] Given 用户运行 `flare dream`，When 主动触发时，Then 立即执行整合并显示进度
...
**Story 4.1**: 作为用户，我希望通过简洁的 Web 界面与 Flare 交互，这样我无需记忆复杂的命令。
**Acceptance Criteria**:
- [ ] Given 我打开 Flare Web，当页面加载时，Then 显示对话界面，包含输入框和历史记录
- [ ] Given 我输入消息并发送，当 Flare 响应时，Then 支持流式渲染 Markdown 内容
...
**Story 5.1**: 作为高级用户，我希望通过 CLI 快速与 Flare 交互，这样我能集成到我的工作流中。
**Acceptance Criteria**:
- [ ] Given 我在终端运行 `flare chat`，当命令执行时，Then 进入交互式对话模式
- [ ] Given 我在对话中输入问题，当我按 Enter 时，Then Flare 响应并显示结果
- [ ] Given 我希望退出，当我输入 `exit` 时，Then 正常退出对话模式

**Story 5.2**: 作为用户，我希望通过 CLI 管理 Flare 服务，这样我无需手动启动服务器。
**Acceptance Criteria**:
- [ ] Given 我运行 `flare start`，当命令执行时，Then 启动 flare-server
- [ ] Given 我运行 `flare stop`，当命令执行时，Then 停止 flare-server
- [ ] Given 我运行 `flare status`，当命令执行时，Then 显示服务状态和端口信息

---

## 4. Solution Overview

Flare 是一个本地优先的个人 Agent 协调器，通过统一入口管理多个专家 Agent，并通过本地 qmd 文件系统积累长期记忆。其核心价值在于：
...
Flare 采用 Server-Client 分层架构：

```
用户界面（CLI / Web / MCP Clients）
        ↓ HTTP 双向通信
    Flare Server（协调层 + 能力层 + 存储层）
        ↓
    LLM Providers + MCP Servers + 本地文件
```

**Monorepo 结构**:
- `flare-server/`: 核心业务逻辑（MoonBit Native）
- `flare-cli/`: 命令行工具（MoonBit Native + argparse）
- `flare-web/`: Web 界面（Vue3，流式 Markdown 渲染）
...
**决策三：Skill & MCP 生态兼容**
Flare 作为 Agent 协调器，原生支持 Claude Code 的 Skill 和 MCP（Model Context Protocol）生态。
- **理由**:
  - Skill 生态提供了丰富的领域专家能力（代码、写作、PDF、前端等）
  - MCP 协议让 Flare 能连接外部工具和数据源（Chrome DevTools、Obsidian、文件系统等）
  - 用户可直接复用现有 skill/MCP，无需重新开发
  - Flare 可以动态发现、加载、组合 skill/MCP 能力
- **核心能力**:
  - Skill 注册与发现：Flare 维护 skill 目录，根据用户意图自动推荐/加载
...
- **理由**: 让 Flare 成为"越来越懂你"的助手，而非固定不变的工具
- **权衡**: 进化算法需要精心设计，避免错误学习
- **替代方案考虑**: 无进化能力（用户每次使用体验相同）

**决策五：Flare 不做专业操作**
Flare 作为协调器，负责理解意图、分配任务、整合结果，不直接执行专业操作。
- **理由**: 遵循"单一职责"原则，专家 Agent/Skill 可独立优化，Flare 专注协调
- **权衡**: 需要维护多个 Agent/Skill 接口，系统复杂度增加
- **替代方案考虑**: Flare 内置所有能力（复杂度高，扩展性差）
...
| `notification_ops` | 通知 | 用户桌面通知 | 仅 Flare 内部触发 |
...
- **隐私保护**: 用户对话内容仅发送至指定的 LLM 服务商，Flare 不额外上传
...
**首次使用引导**:
1. 用户启动 Flare（CLI 或 Web）
2. 显示欢迎信息："你好，我是 Flare。我会记住我们的每一次对话，越来越懂你。"
...
2. Flare 分析意图：
   ...
   - 如果是通用对话，直接响应
3. Flare 生成响应（流式显示）
4. 对话结束后，Flare 后台提取关键信息存储到 qmd 文件
...
- API 密钥无效："API 密钥无效，请运行 `flare config` 重新设置。"
...
- [ ] Given 运行 flare-server，When 服务器启动，Then 监听指定端口（默认 3000）
...
**Acceptance Criteria**:
- [ ] Given 运行 `flare start`，When 命令执行，Then 启动 flare-server 并显示状态
- [ ] Given 运行 `flare chat`，When 进入对话模式，Then 显示交互式提示符
- [ ] Given 在 CLI 中输入问题，When 按 Enter，Then 显示 Claude 响应
- [ ] Given 打开 flare-web，When 页面加载，Then 显示对话界面
- [ ] Given 在 Web 中输入问题，When 发送，Then 显示 Claude 响应
- [ ] Given skills 目录中有 skill 文件，When Flare 启动时，Then 自动扫描并注册可用 skills
- [ ] Given 配置 MCP server，When Flare 启动时，Then 能连接并列出可用 MCP tools
...
**Acceptance Criteria**:
- [ ] Given 对话结束，When Flare 分析对话，Then 生成 qmd 记忆文件
- [ ] Given 用户提问，When Flare 检索记忆，Then 引用相关 qmd 文件内容
...
- [ ] Given 用户提出代码问题，When Flare 分析意图，Then 自动路由到 code skill
- [ ] Given 用户提出 PDF 问题，When Flare 分析意图，Then 自动路由到 pdf skill
...
- [ ] Given 用户纠正 Flare 回答，When 反馈时，Then 更新相关知识
- [ ] Given 用户提出代码问题，When Flare 分析，Then 调用代码 Skill 并返回结果
- [ ] Given 用户提出写作问题，When Flare 分析，Then 调用写作 Skill 并返回结果
- [ ] Given 用户提出涉及多个 Skill 的需求，When Flare 分析，Then 协调多个 Skill 协作
- [ ] Given 统计用户 Skill 使用频率，When 归纳时，Then 生成"最常用 Skill"报告
- [ ] Given 用户在 Obsidian 中安装插件，When 插件启用，Then 能调用 Flare 服务
- [ ] Given 在 Obsidian 中选中文字，When 运行 Flare 命令，Then 发送至 Flare 并显示响应
...
| Product | Strengths | Weaknesses | Differentiation from Flare |
|---------|-----------|------------|----------------------------|
| Claude Code | 强大的代码能力，上下文窗口大 | 垂直代码场景，普通用户难用 | Flare 面向通用场景，更低门槛 |
| ChatGPT | 广泛使用，生态丰富 | 无本地记忆，隐私风险 | Flare 本地记忆，隐私保护 |
| Obsidian + AI 插件 | 本地优先，知识管理强 | AI 能力有限，无 Agent 协调 | Flare 专注 AI 协调，记忆积累 |
| Cursor | 代码编辑器集成 AI | 仅限代码场景 | Flare 通用场景 |
